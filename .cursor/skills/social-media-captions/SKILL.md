...
- Dumping all interview questions in one message instead of asking one at a time
...
Ask **one question at a time**. Do not list multiple interview prompts in a
single message. Start with what feels most missing after reading the code;
wait for the answer, then ask the next gap. Keep it conversational.
